...
while 0 < v(end)
    test_frottement=g-parameters.CLmax*0.5*parameters.rho*S*(v(end)^2)/m;
    if test_frottement <0
        ...
        dv=(dt*(0.5*parameters.rho*S*(parameters.f*parameters.CLmax-parameters.Cd)*(v(end)^2)/m)-parameters.f*g*dt);
        v=[v dv+v(end)];
        t=[t t(end)+dt];
        x=[x (((v(end)+v(end-1))*dt/2)+x(end))];
    end
